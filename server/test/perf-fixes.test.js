...
const DB_PATH = path.join(os.tmpdir(), `perf-fixes-${Date.now()}-${Math.random().toString(36).slice(2)}.db`);
...
describe('chat.js: pedido de un solo producto queda en order_items', () => {
  test('_createOrder (via flujo "falta direccion") inserta orders + order_items', async () => {
    ...
    db.prepare(`INSERT INTO pending_orders (phone, product_id, product_name, missing_field) VALUES (?, ?, ?, 'address')`)
      .run(`app:${username}`, product.body.id, product.body.name);
    ...
    const items = db.prepare('SELECT * FROM order_items WHERE order_id=?').all(res.body.order_id);
...
describe('users.js: borrar usuario con historial da error claro, no 500', () => {
  test('worker con pedido/login asociado no se puede borrar (409, no 500)', async () => {
    ...
    const db = getDB();
    const user = db.prepare(`SELECT id FROM users WHERE username='worker_con_historial'`).get();
    expect(db.prepare(`SELECT COUNT(*) c FROM login_events WHERE user_id=?`).get(user.id).c).toBeGreaterThan(0);
